      ...
      <div className="lg:col-span-2 space-y-6">
          ...
          {tabs.map((tab) => (
            ...
            >
              ...
              {activeTab === tab.id && (
                <motion.div
                  layoutId="activeTab"
                  className="absolute inset-0 bg-lime rounded-2xl -z-10"
        ...
        <div className="bg-gray-800/50 backdrop-blur-sm rounded-3xl p-8 lg:p-10 border border-gray-700/50 min-h-[250px]">
          <AnimatePresence mode="wait">
            ...
            >
              <h4 className="text-lime font-medium mb-4 text-lg">
                {tabs.find(t => t.id === activeTab)?.label}
              </h4>
              <p className={`text-lg leading-relaxed ${
                activeTab === 'result' ? 'text-white font-medium' : 'text-gray-300'
              }`}>
                {getContent()}
                {activeTab === 'result' && (
                  <span className="text-lime font-bold block mt-4">
                    Uniknięcie straty szacowanej na ok. 200 tys. PLN.
                  </span>
                )}
              </p>
            </motion.div>
          ...
          {tabs.map((tab, index) => (
            <button
              ...
              className={`h-1.5 rounded-full transition-all duration-300 ${
                activeTab === tab.id ? 'w-8 bg-lime' : 'w-2 bg-gray-600'
              }`}
            />
          ))}
        </div>
